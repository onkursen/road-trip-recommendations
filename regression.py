...
desired_attributes = open('desired_attributes.txt').read().split('\n')
...
def score(featureVector):
	...
	return exp_rating_metric(rating) * gaussian_distance_metric(added_distance) * desired_attributes_metric(attributes, rating)
...
print 'This adds a distance of %f km with a %.1f-star rating' % (test_set_features[recommended_index][0], recommended['stars'])
